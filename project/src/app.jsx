/* Draft Era — main app: two-player draft flow, lifelines, scoring. */
const { useState, useEffect, useRef, Fragment } = React;

const ACCENTS = ["#FF6A1A", "#2D8CFF"];
const GM_NAMES = ["GM ONE", "GM TWO"];
const POS_LIST = window.POSITIONS; // [{id,name}...]
...
function App() {
  ...
  const [targets, setTargets] = useState({ t: randInt(window.TEAMS.length), d: randInt(window.DECADES.length) });
  ...
  const [justFilled, setJustFilled] = useState(null); // {gm,pos}
  ...
  useEffect(() => () => { clearTimeout(spinTimer.current); clearTimeout(toastTimer.current); }, []);

  const activeGM = turn % 2;
  const roster = current ? window.rosterFor(current.abbr, current.decadeId) : [];
  ...
  function doSpin() {
    ...
    const t = randInt(window.TEAMS.length);
    const d = randInt(window.DECADES.length);
    setTargets({ t, d });
    setSpinKey((k) => k + 1);
    setPhase("spinning");
    setSelected(null);
    ...
    spinTimer.current = setTimeout(() => {
      const team = window.TEAMS[t];
      const dec = window.DECADES[d];
  ...
  function placeAt(pos) {
    if (!selected) return;
    const ev = window.SCORING.evaluatePick(selected, pos, roster);
    setPlayers((prev) => {
      const next = prev.map((pl, i) => {
        if (i !== activeGM) return pl;
        const lineup = Object.assign({}, pl.lineup);
        lineup[pos] = { player: selected, eval: ev, slot: pos };
        return Object.assign({}, pl, { lineup });
      });
      return next;
    });
    setLastEval(ev);
  ...
  function useRespin() {
    ...
    setPlayers((prev) => prev.map((pl, i) => i === activeGM ? Object.assign({}, pl, { lifelines: pl.lifelines - 1 }) : pl));
  ...
  // ---- derived ----
  const wins = players.map((p) => window.SCORING.projectWins(p.lineup));
  const filledCount = (p) => POS_LIST.filter((q) => p.lineup[q.id]).length;
...
function Center(props) {
  ...
  const team = current ? window.teamByAbbr(current.abbr) : null;
  const dec = current ? window.decadeById(current.decadeId) : null;
      ...
      {(phase === "spin" || phase === "spinning") && (
        <div style={{ display: "contents" }}>
          <div className="reel-card">
            ...
            <div className="reels">
              <Reel items={window.TEAMS} target={targets.t} spinKey={spinKey} kind="team" durationMs={2300} />
              <Reel items={window.DECADES} target={targets.d} spinKey={spinKey} kind="decade" durationMs={2900} />
            </div>
...
function GameOver({ players, wins, onNew }) {
  ...
  const verdict = window.SCORING.finalVerdict(a, b, players[0].name, players[1].name);
