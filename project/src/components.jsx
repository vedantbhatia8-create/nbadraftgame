/* Presentational components for Draft Era. Exported to window at the end. */
...
/* ---------- Slot-machine reel ---------- */
function Reel({ items, target, spinKey, kind, durationMs }) {
  ...
  const curIndexRef = React.useRef(target || 0);
  const [ty, setTy] = React.useState(rest(target || 0));
  const [trans, setTrans] = React.useState("none");
...
/* ---------- A player row in the search list / candidate list ---------- */
...
/* ---------- A lineup slot ---------- */
function Slot({ posId, posName, entry, isTarget, justFilled, accent, onClick }) {
  const filled = !!entry;
  const cls = ["slot"];
...
function Host({ text, grade, gradeColor }) {
  return (
    <div className="host">
...
Object.assign(window, { BasketballIcon, MicIcon, SearchIcon, Reel, PlayerRow, Slot, Host });
